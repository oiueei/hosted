...
const ME = { code: 'ME0001', name: 'Carlos', email: 'me@test.com', koro: 'basic', created: '2026-01-01' };
...
const setApi = ({ memberships = [], profile = ME, invitedOk = true } = {}) => {
  apiFetch.mockImplementation((url) => {
    if (url.startsWith('/api/v1/invited-collections/')) {
      return invitedOk ? ok(memberships) : Promise.resolve({ ok: false, status: 500, json: async () => ({}) });
    }
    return ok(profile);
  });
};
...
const renderOwn = () => render(<MemoryRouter><UserPage /></MemoryRouter>);
const renderOther = () =>
  render(
    <MemoryRouter initialEntries={['/OTH001']}>
      <Routes><Route path="/:userCode" element={<UserPage />} /></Routes>
    </MemoryRouter>
  );
...
describe('UserPage — My groups', () => {
  test('lists the groups I belong to, each with its own way out', async () => {
    ...
    expect(screen.getByRole('link', { name: "Lili's Lending Library" }))
  ...
  test('a localized group name is resolved, never raw JSON', async () => {
    setApi({ memberships: [{ code: 'COL003', headline: '{"en": "Mum\'s things", "es": "Las cosas de mamá"}' }] });
