...
export default function BackLink({ to, href, label }) {
  ...
  // the URL and only passes `href` when it's a real http(s) address. It carries
  // no external-link icon on purpose: the caller wants it to read, and look,
  // exactly like the internal link.
  if (href) {
    return (
