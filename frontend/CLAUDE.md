...
**Contact corner (i8):** every hero also carries `ContactCorner` (`src/components/ContactCorner.jsx`) — a quiet `IconSpeechbubbleText` link to `/contact` (44px target, `aria-label`/`title` from `contact.linkLabel`), rendered as the first child of `.form-hero-content` (so `--hero-text-color` resolves and it stays visible on dark theeemes, like the back link) and absolutely positioned against `.form-hero` at the content column's right edge (`.contact-corner` in App.css). Inserted in `PageLayout` plus the 8 manual-hero pages.
