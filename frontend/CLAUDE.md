...
| `/owner-bookings` | `OwnerBookingsPage` | The owner's mirror: every request made **on their things**, answerable from here (accept / reject per row). Accepting a GIFT/SELL that isn't endless opens a confirmation `Dialog` first — that acceptance hands the thing over permanently (`thing_is_endless` on the booking serializer is what distinguishes it from an endless gift, which transfers nothing). Closes the asymmetry where the requester had a list and the owner had only the email, an inbox banner, or opening each collection. Uses `GET /api/v1/owner-bookings/`, which since 2026-09 also returns every booking on a thing in a **PROPRIETARY** collection the viewer curates — so a co-curator decides its holds and cancels its reservations here too (the page is data-driven; the accept/reject/cancel controls render per row, the backend gates the action). Because that pools requests across several groups, each row now shows its **`collection_headline`** under the thing name and the thing link carries `/collections/{collection_code}/things/{thing_code}` when the group is known, so a co-curator of more than one can tell the rows apart. A requester with no name renders as `common.aMember` (the "Asked by" line always shows now, rather than vanishing). Linked from HomePage when the user **owns a thing** (a COMMUNITY member can be asked for a contribution) **or curates a PROPRIETARY collection** (`myCollections.some(c => c.mode === 'PROPRIETARY' && c.is_curator)`) |
...
| `/collections/:code/join` | `JoinPage` (fetches the public collection itself so its name survives a refresh or a shared /join URL — `ThingLinkbox` is the only caller that passes it in navigation state, so the hero's own "Join to take part" link used to ask a stranger to join "Collection") | **Public route** — login-to-act landing for a PUBLIC collection. An anonymous visitor who clicks an action button (Claim / Buy / Rent / Borrow) on a public collection lands here; enters email → `/auth/join/` joins them to `:code` + magic link → after verifying they're dropped back on the collection, able to act. When the click came from one specific thing the URL carries `?thing=<code>` (S13) — passed through the join and back on the RSVP, so verifying lands them on `/collections/:code/things/:thing`, not the grid. |
...
**Contact corner (i8):** every hero also carries `ContactCorner` (`src/components/ContactCorner.jsx`) — a quiet `IconSpeechbubbleText` link to `/contact` (44px target, `aria-label`/`title` from `contact.linkLabel`), rendered as the first child of `.form-hero-content` (so `--hero-text-color` resolves and it stays visible on dark theeemes, like the back link) and absolutely positioned against `.form-hero` at the content column's right edge (`.contact-corner` in App.css). Inserted in `PageLayout` plus the 8 manual-hero pages. On `CollectionPage` a curator also gets `ShareCollectionMenu`'s `IconShare` one 44px slot to its left (`.share-corner`).

**OIUEEI logo in the hero (S9):** the hero once carried a 40px `::after` logo watermark at the content column's right edge on every page. It was removed in 2026-09 (S12) — it competed with the collection's own title for attention, the brand is already in the footer colophon (`SiteFooter`), and dropping it freed the top-right corner for a discreet share control. What remains of S9 is the **title replacement**: the one hero `<h1 class="form-hero-title">` whose text is the *literal* string "OIUEEI" (only `login.title` — `share.pageTitle` is "Join us on OIUEEI", `notFound.title` is "Page not found", neither qualifies) renders `.form-hero-title-logo` (80px masked `oiueei-logo.svg`, `var(--hero-text-color)` — white on `/login`) instead of the text, with `aria-label={t('login.title')}` on the `<h1>` so the accessible name survives. `LoginPage` used to also carry a `form-hero--no-watermark` class to suppress the watermark next to that logo; with no watermark left, the class and its CSS are gone too.
...
- **Hero title is the OIUEEI logo (S9)**: the only hero `<h1>` in the app whose text is the literal string "OIUEEI" — it renders `.form-hero-title-logo` (an 80px masked `oiueei-logo.svg`, coloured via `--hero-text-color`) instead, with `aria-label={t('login.title')}` on the `<h1>` for the accessible name.
...
- **ACCOUNT_DELETE is the exception to the auto-commit**: when the GET preview's `action` is `ACCOUNT_DELETE`, the page renders a dedicated confirmation screen (account email, owned collection/thing counts, the what-stays line) and the committing POST only fires from the explicit **"Delete my account forever"** danger button — never from the load effect. On success it clears `userCode`/`theeemeColors`/`koro` from localStorage and shows the goodbye screen (`verify.accountDeleted*`); a Cancel button exits to home/login.
...
- On success: stores `userCode` in `localStorage`. Auth tokens are set as HttpOnly cookies by the backend. **The destination comes from the backend's `landing` field** (see `core/views/CLAUDE.md` → VerifyLinkView): `collection` ⇒ `/collections/{data.collection || data.invited_collection}`, `welcome` ⇒ `deployment/aboutPath` (upstream null, so it falls through to `/` — nothing here produces that landing anyway), anything else ⇒ `/`. It used to be decided here from a `seenWelcome` localStorage flag, but `LogoutPage` cleared that key, so every re-login looked like a first visit; moving the decision to the server is what fixed it, and the flag is gone entirely now that the collection welcome box it fed is too (S11).
...
- **Hero back link:** normally an internal `<Link to="/">` labelled `common.home` ("Home"). When `collection.home_page` is set and `sanitizeUrl` accepts it (http/https only), `BackLink` gets an `href` instead and renders a plain external `<a rel="noopener noreferrer">` **with an `IconLinkExternal` after the label**, and `CollectionPage` swaps the label to `collectionPage.backToSite` ("The group's site") — it leaves the app, so it must not read as "back to the OIUEEI home". A non-http value falls back to the internal link and label.
- **`isCurator`, not client-side `isOwner`, gates every admin control now.** The page derives a strict `isOwner` (`userCode === collection.owner`) only for the founder-only pieces below (attribution, delete-adjacent controls); everywhere else it reads the server-computed `collection.is_curator` — true for the owner **or** a co-owner (2026-09, co-curators). This is the one field on `CollectionSerializer` that actually says who may act, rather than the page comparing `owner` itself.
- **"Edit collection" button** visible to a curator (owner or co-owner), links to `/collections/{code}/edit`.
- **"Add thing" button** visible to a curator (always) and to invited users in COMMUNITY mode, links to `/collections/{code}/add`.
- **"Manage guests" button** visible to a curator, links to `/collections/{code}/invites`.
- **Community tag**: when `collection.mode === 'COMMUNITY'`, an HDS `Tag` with "Community" label is shown next to the headline.
- **Attribution line (2026-09, co-curators in PROPRIETARY)**: one line under the description, both modes.
  - **With co-curators** (`collection.co_owners.length`): `Curator:` — actually the `curatorsLabel` key, "Co-curators:" / "Co-curadores:" / "Co-curadors:" — then the **founder first**, then each co-curator, comma-separated, everyone at the same level and each linked to `/{code}`. Shown to **everyone, including the founder** (it replaces the old separate `Co-curators.` line, which was already public). Names come from `owner_name` + the public `co_owners` list.
  - **Without co-curators**: the single founder line, `collectionPage.curator` ("Curator:" — colon, and the same word in **both** modes now; the old mode ternary and the `owner` / `coOwnersLabel` keys are retired), shown only to a **non-owner** (the founder knows who they are).
- **INACTIVE notice**: when the collection status is `INACTIVE` and the viewer is a curator, a `Notification` informs them "This collection is inactive. It is not visible to guests." Guests cannot access inactive collections (backend returns 403).
- **Inbox notifications (any member)**: `<InboxNotifications collection={code} />` at the top of the page content, above the banners, shown to a curator **and** any invited member (`isCurator || collection.is_member`). A hold request or a FAQ question is answered on the thing, so it must reach whoever owns *that* thing where it lives — not only on Home (O1). Scoped to the collection via the endpoint's `?collection=` filter, which the backend also always scopes to the requesting user, so a member only ever sees their own notifications (e.g. bookings on a COMMUNITY contribution they own) — never a co-member's or the owner's. Gating on membership rather than `isOwner` alone matters in COMMUNITY mode, where a thing's owner and the collection's owner can differ; it was owner-only until that gap surfaced (2026-09), and a co-owner needed the same widening the same round.
- **Pause banner**: when `collection.is_paused` is true, a fixed non-dismissible HDS `Notification` (type `alert`) is shown at the top of the page content area, with label `pause.bannerLabel` and body `collection.pause_message`. Shown to both curators and guests. `isPaused={collection.is_paused}` is passed to every `ThingLinkbox` so Hold buttons are disabled while paused.
- **Share menu**: a single discreet `IconShare` at the hero's top-right, immediately after `<ContactCorner />` in `.form-hero-content`, shown to a curator (S9, 2026-09 — it was a full-width HDS `Select` under the curator buttons until then). Renders `<ShareCollectionMenu>`; `.share-corner` in App.css restyles the Select's own trigger to a 44px icon button (see the component note). It receives `isPublic={collection.visibility === 'PUBLIC'}`. For a **PRIVATE** collection it calls `POST /api/v1/collections/{code}/share-link/` on first interaction to lazily generate the public token and shares the `/share/{token}` join URL (recipient must enter their email to join). For a **PUBLIC** collection it **skips the token entirely** and shares the collection page directly (`${window.location.origin}/collections/{code}`) — anyone can read it without an account, so no email gate; a visitor who wants to *act* is asked to log in only then (login-to-act). Either way the resolved URL is cached in a `useRef` and dispatched to the chosen action: `mailto:`, `navigator.clipboard.writeText`, `https://wa.me/?text=`, or the QR dialog. Email subject/body and WhatsApp text are pre-filled with the collection headline and the URL, translated to the acting curator's language.
- **Digest switch**: under the attribution block, shown when `!isOwner` (the founder never receives their own group's digest) **and** (`collection.is_member` **or** `isCurator`) **and** `digest_frequency !== 'NONE'` — a co-owner is still an ordinary `invites` row for the digest, so this reads separately from the rank-and-file member block below rather than nesting inside it. One line of state plus a text `<button>` (`.digest-pref` / `.digest-pref-button`) that POSTs `{muted}` to `/api/v1/collections/{code}/digest/` and only moves the label once the server agrees, so a failure can't leave it lying. It is the per-group half of the email preferences: `User.notify_news` is the master switch on the profile, this narrows it to one collection — together they're what allow news to default ON without a pre-ticked opt-in (DESIGN §6).
- **Recommend a guest (rank-and-file members only)**: `RecommendGuest` in the hero, shown when `!isOwner && is_member` **and** `collection.allow_member_proposals` — a curator already has the real invite one control over, so this doesn't offer itself to one (`is_member` is `false` for a co-owner by design). A quiet one-line link that expands into email + optional note, posting to `/collections/{code}/invite/propose/`. The copy leads with "{owner} decides — nothing is sent to them until they say yes", because a member who walked away thinking they had invited somebody would have been misled by us. Called **recommend**, not invite or propose: the verb carries that you are putting your name behind the person, which the invitation itself will say if a curator agrees.
- **Broadcast section**: shown to a curator when the collection has invitees. A "Send a message to guests" button opens an inline form with just a message (TextArea, max 256) field — the subject is auto-generated server-side as `Hey! {collection}`. Its **helper line discloses the Reply-To** (`broadcast.replyToNotice`): the email carries the sender's own address (the acting curator, founder or co-owner) so a reply is one tap, which makes this the one screen where a member learns an address the API otherwise never serves them. Said before the send, not after (DESIGN §6) — and said rather than switched off, because a broadcast with no reply path is a megaphone. Submits to `POST /api/v1/collections/{code}/broadcast/`. Shows success/error Notification inline. Closable via "Close" button.
- **Things section**: shows all non-inactive things for both curators and guests (responsive 3-column grid), **capped at `CARDS_PER_PAGE` (24) rendered cards** with a "Show N more things" button below (design round). The collection serialises *every* thing it holds and `COLLECTION_THINGS_BLOCK` is off by default, so a 200-item lending library used to mount 200 `ThingLinkbox`es — each with its own theeeme, localisation and booking view-model — on first paint (DESIGN §7; the photos were already `loading="lazy"`, so what this cuts is mount cost, not downloads). It is a **render** cap, not server pagination, because the tag chips count across the whole collection and a paginated payload would make those counts lie. Picking a tag resets the count.
- **"Leave the group" is no longer here.** It used to sit in the hero, third in a stack of unlabelled text links under the description (recommend → digest → leave) and the only destructive one of the three. It moved to the own profile's "My groups" list (`UserPage`) in the 2026-08 design round: leaving is something you do to your own membership, so it belongs with the rest of your account, next to the other memberships you might weigh it against. The route (`/collections/:code/leave`) and `LeaveCollectionPage` are unchanged. A curator is refused this route server-side (2026-09) — a co-owner leaves by being demoted, not by self-unlinking — and the button never offered itself to one anyway, since it keys on `is_member`.
...
- **Community attribution** (before headline, COMMUNITY collections only): when `collectionMode === 'COMMUNITY'`, renders a `thing-card-meta` paragraph showing `owner_name` — linked to the member's profile (`/{thing.owner}`, `.thing-card-owner-link`) — and the creation date (`formatDate`, `DD/MM/YYYY`). A **signed-out** reader is told there is a contributing member but not which one: the API withholds that name (it belongs to a group, not to the open web — same rule as the journey below), so the paragraph renders `common.aMember` ("A member") as **plain text, not a link** — the profile behind it is `IsAuthenticated`, so for that reader the link was only ever a door onto a 403. Uses the `collectionMode` prop passed from `CollectionPage`.
...
- **Anonymous login-to-act**: for a signed-out visitor the reserve / answer buttons are shown (via `useThingActions`' `loginToAct` option) but each click **navigates to `/collections/:code/join?thing=:thingCode`** (`JoinPage`) instead of acting — the same pattern as `ThingLinkbox` on `CollectionPage`, and the `?thing=` is what brings the magic link back to this page (S13). (The old inline `JoinToAct` box was removed.) Member-only sections (FAQ ask form, report footer) stay hidden until they log in.
...
- **The collection's request-page note** — a Markdown-editable note the owner writes once for the whole collection (`Collection.request_info`, set in Create/Edit under "More options", `LocalizedInfo variant="requestInfo"`), shown for **every** date-based verb reaching this page (LEND/RENT/RESERVE), not RESERVE_THING only. Read via `thing.collection_request_info` (resolved through `useLocalized`'s `L`) and passed as `PageLayout`'s `description` prop, rendered via `<MarkdownText>` — `PageLayout` renders `description` in a `<div className="form-hero-text">` rather than a `<p>` specifically so Markdown's block-level output (lists, headings) is legal there. Absent (`undefined`) when the collection has no note, so nothing renders.
- **Form fields** adapt to thing type:
  ...
  - `RESERVE_THING` — **two flows, chosen by `thing.reservation_unit`, never both.**
    - **`DAY`** (the original shape) — **pickup date + duration in days** (1..`thing.reservation_max_days`; the duration `Select` is omitted and the length fixed to 1 when the cap is 1), plus an optional **`project_note` `TextArea`** (≤512). The date picker's `maxDate` is `today + thing.reservation_horizon_days` (the collection's own limit, not the fixed 90 the rental picker uses) and its `dateOutsideRangeErrorText` names that number (`reservation.dateRange` with `{days}`, not the generic `request.dateRange`'s hardcoded 90). A `reservation.pickupHelper` line under it says closed/booked days are greyed out (the member sees a half-dead calendar and, without it, no reason). Pickup picker uses `reservationPickupDisabled` (`utils/rental.js`) — stricter than `isPickupDisabled`: **every** day of the span must be an allowed weekday, since the space can't be booked across a closed day. When the length is more than a day and a pickup is set, a `reservation.dateSummary` line shows the inclusive span it will book (`start` … `start + duration − 1`) — the reservation auto-confirms, so there is no owner step to catch a wrong end. POSTs `{ start_date, duration_days, project_note, collection_code }`.
    - **`HOUR`** — day first, **then** duration, **then** start time, each step narrowing from the one before (unlike `DAY`'s length-first order): a `DateInput` (`#reservation-pickup-date-hourly`) disabled by `isHourlyPickupDisabled` (`utils/rental.js` — closed weekday, a `closed_dates` entry, or no duration/start combination leaving even one free hour that day); once a day is picked, a `RadioOptionGroup` of `durationOptions(dayBlocks(...), thing.reservation_max_hours)` — every whole hour up to the cap, plus "half day" (the longest single block, 2+ blocks only) and "full day" (the day's whole span, any gap included) whenever each *itself* fits under the cap, since **the cap applies to those two forms too, with no exception** (CA's call, mirroring `Collection.reservation_hour_violation`); once a duration is picked, a second `RadioOptionGroup` of `freeStartTimes(...)` — every hour-stepped start that duration still fits, computed from the day's own booked ranges (`dayBookings`), with a `reservation.noStartTimesForDuration` fallback `Notification` when none remain (pick a shorter one) — and the same `project_note` field. Picking a new day resets both radios; picking a new duration resets the start-time one. POSTs `{ start_date, start_time, end_time, project_note, collection_code }` — `end_time` is derived client-side (`formatHM(parseHM(start_time) + duration.minutes)`), never sent as a duration.

    Either way, on success the notice reads *"Your reservation is confirmed"* — it is auto-confirmed, so there is no "we've let the owner know".
...
- **`formatDate(value)`** (`utils/rental.js`) — the one way the app renders a date it *displays*: `DD/MM/YYYY` for every locale (es/ca/en), matching the date pickers and `closed_dates`. Accepts an ISO date, an ISO datetime or a `Date`; `''` for anything unparseable (a null booking range shows blank, never "Invalid Date"). Replaced the bare `toLocaleDateString(i18n.language)` calls, which handed `en` readers American `MM/DD/YYYY`. Used on ThingPage (created, journey `lent_date`/`returned_date`), My/OwnerBookings (`_created`, ranges), `OwnerBookingsList`, `ThingLinkbox`, `ThingInfoRows` + RequestThingPage (`next_available`), UserPage ("member since"). Backend emails match it via `email_service._fmt_date` (see `core/services/CLAUDE.md`).
- **`formatBookingWhen(booking)`** (`utils/rental.js`) — a listing row's whole date/time range, reading the API's own field names (`start_date`/`end_date`/`start_time`/`end_time`) directly. A whole-day booking (`start_time` absent — every LEND/RENT, every `DAY`-unit RESERVE) renders `'DD/MM/YYYY — DD/MM/YYYY'`, unchanged from before this helper existed; an `HOUR`-unit RESERVE_THING slot renders `'DD/MM/YYYY, HH:MM–HH:MM'` — the date once, both clock times, never the day-based `end_date` (which for an hourly booking is only `start_date + 1`, the same "free again" marker the backend's calendar export and reminders read, not a real second day). `''` when there are no dates at all (GIFT/SELL). Shared by `MyBookingsPage`, `OwnerBookingsPage` and `OwnerBookingsList` — one definition so the three tables can't render an hourly slot three different ways.
- **Buttons**: Cancel (navigates back) + Hold / Reserve (submits request).
- On success: shows an inline HDS `Notification` ("You're all set! We've let the owner know — they'll get back to you soon.", or "Your reservation is confirmed" for RESERVE) with a "Back to {backLabel}" button. Does not navigate automatically.
- On error: toast notification (top-right, auto-close) — **except** RESERVE_THING's "not a member" 403, handled by `joinThenRetry` (CA's call): that specific rule failure can only fire on a **PUBLIC** collection (`can_view` already gated everything else, and a non-member reaches it only there), so joining is always genuinely possible and was never really a choice being offered — the page auto-joins and silently retries the reservation, with no second click and no "you're in, try again" step in between. **Gated on the backend's `code: "not_a_member"` marker, not the bare 403 status** — `BookingRequestError.code` (`core/services/booking_service.py`), since this endpoint's *other* 403 (`get_viewable_thing`'s "not authorized to request this thing" — e.g. the thing going INACTIVE while the form was open) carries no such code and must not be mistaken for a membership gap; auto-joining on that one would silently add the reader to a group over an unrelated error. It `POST`s `/api/v1/collections/{code}/join/` — the same signed-in half of login-to-act `CollectionPage`'s `handleJoin` already uses, reused rather than reinvented (`code` covers the collection-context route; the standalone `/things/:code/request` route falls back to `thing.collection_code`, the collection `_viewable_collection` already resolved server-side). **Only a failure of the auto-join itself surfaces anything**: a persistent `.invite-nudge` block (not a toast, which would auto-close before the reader could act) shows the backend's original reason plus a manual **"Join this group"** fallback button (`handleJoinGroup`) that retries the same join-then-reserve sequence — **rebuilding the request body fresh** (`buildReservationBody()`, shared with the normal submit path) rather than replaying the failed attempt's, since the reader may have changed the date while the fallback was showing and a stale body would confirm the wrong one. A failed retry shows its own inline error (`collectionPage.visitorJoinError`) and leaves the original reason in place. Any 403 without the marker — from either attempt — gets the plain toast.
...
- **API:** `GET /api/v1/share/:token/preview/` on mount, then `POST /api/v1/auth/join/` with `{ email, share_token }`.
...
- **Names the collection (S10).** The generic "Join us on OIUEEI" tells a stranger handed the link on WhatsApp nothing. On mount `SharePage` GETs `/share/:token/preview/` (public, returns only `headline` + `description`, both raw — resolved here through `useLocalized`, O6) and, once it lands, passes `titleText` / `descriptionText` / `docTitleText` and `collectionDescription` down so the hero reads "Join {name}" with the owner's own description under it (`share.pageTitleNamed` / `pageDescriptionNamed` / `titles.shareNamed`). Best-effort: a 404 (unknown / revoked token — the endpoint 404s those deliberately) or a network error just leaves the generic copy, never a visible error — the join form works regardless. `MagicLinkJoinPage` falls back to its `*Key` props whenever the `*Text` override is absent.
...
- **Gated on `is_curator`, not `isOwner`** (2026-09, co-curators): the whole management UI (invite form, bulk CSV, the roster's actions column, the proposals block) reads the server-computed `is_curator`, so a co-owner sees exactly what the founder does. A separate strict `isOwner` (`userCode === data.owner`) is kept only for the one thing that stays founder-only — promoting/demoting.
- Simple page with h1 title:
  - Lists current invites by name/email. Pending invites show "Pending" label with "Resend" and "Remove" buttons. A curator sees "Remove" (and, if pending, "Resend") per row. A co-owner's own row carries a "Co-curator" status instead of "Accepted" (`manageInvites.coOwnerStatus`), read from the `co_owners` list `CollectionSerializer` now serves.
  - **Promote/demote (any curator, 2026-09)**: on each accepted (non-pending) row, a curator — owner or co-owner, `isCurator` now, not the old founder-only `isOwner` — gets a star toggle (`IconStar`/`IconStarFill` `TooltipButton`, "Make co-curator" / "Remove co-curator status") that `POST`s or `DELETE`s `/api/v1/collections/{code}/co-owners/` with `{user_code}`, then reloads the collection so the roster's badge and the co-owner set stay the server's own word. The founder's own row is never in `invites`, so it never carries the toggle — they cannot be demoted. **Promoting opens a confirm `Dialog` first** (design round): co-curator status hands over nearly the founder's whole reach — the member list with emails, the power to appoint more co-curators — and makes the person's name public on the group's page, so it is not a one-click toggle (`manageInvites.promoteConfirm*`, DESIGN §6). Demoting only removes access and stays one click.
  - A curator sees email input + "Invite" button below the guest list.
...
- **Two-tier form (progressive disclosure, design round O1)** with h1 title. The **visible** tier (a `form-grid`) carries the happy path and anything that can block submit: `TextInput` for headline (required), `TextArea` for description (+ their `LocalizedInfo` hint), `Select` for status (ACTIVE/INACTIVE), the mode `RadioButton` group, and the `CollectionForm` identity cluster (visibility toggle + the recommend-a-guest toggle + the allowed-thing-types multi-select — the mode-gated swap/share toggles went with those types). Everything **optional-with-a-safe-default** folds into a single collapsed HDS `Accordion` ("More options", `createCollection.advancedTitle`, `language="en"`, closed on load, heading tinted with theeeme `color_04` via the `--header-color` theme token), in this order: tags editor (`TagInput` + `LocalizedInfo variant="tags"`), **rental rules** (`RentalRulesFields`), digest `Select` (None/Weekly/Monthly), collection-language `Select`, thumbnail `ImageUpload`, welcome-doc `PdfUpload`. Nothing inside the accordion is required or can block submit (`validate()` only checks headline + description length + "pick at least one type", all visible), so the form is completable without ever opening it.
  ...
  - **Rental rules (#7):** `RentalRulesFields` (extracted from `CollectionForm` in O1) via `utils/rental.js`, rendered inside the accordion — a `Select multiSelect` for rental lengths plus the **`WeekdayChips`** row for pickup/return days. They save `rental_durations` (days) + `rental_weekdays` (0=Mon…6=Sun). Below the weekday chips, a `TextArea` for `deposit_policy` (S6, D5, 2026-08) — how deposits work in this group, in the owner's own words; empty by default, localizable like every other owner text (`LocalizedInfo variant="policy"`), and deliberately not a per-thing toggle since the amount already lives on each thing.
  - **`WeekdayChips`** (`src/components/WeekdayChips.jsx`) — the `[L M X J V S D]` toggle row shared by `RentalRulesFields` and `ReservationRulesFields` (they carried a byte-identical copy each): `.weekday-chips`, accessible toggle `<button>`s with `aria-pressed` + full-name `aria-label`, narrow letters via `Intl`. Caller passes `labelKey` / `helperKey` / `labelId`, the `weekdays` value + setter (Python 0=Mon…6=Sun, kept sorted), and the theeeme token names `color01` / `color06` for a **selected** chip — fill + text, the primary-button pairing that `paletteContrast.test.js` pins at ≥ 4.5:1 on all 12 palettes (white text on a light `color01` like engel was 1.22:1). No theeeme in hand ⇒ the App.css default (white on `--color-black-90`) stands.
  - **`OpeningHoursField`** (`src/components/OpeningHoursField.jsx`) — the weekly opening-hours editor for an HOUR-unit reservations collection, rendered by `ReservationRulesFields` when `reservation_unit === 'HOUR'`. A single raw-JSON `TextArea` matching `Collection.opening_hours` exactly — `{"0".."6": [["HH:MM","HH:MM"], ...]}`, Python weekday() numbering (0=Mon…6=Sun) as string keys, a day absent or `[]` closed. **CA's own call**, after trying a per-day block editor first: typing or pasting the JSON directly beats clicking through seven fieldsets for a schedule already written down. `ClosedDatesField`'s stance mostly holds — the deeper shape checks (day keys, HH:MM format, block overlap) stay the backend's job (`_validate_opening_hours`), surfaced as its own 400 via Toast — but this field **cannot** forward unparsed text the way `ClosedDatesField` does: `opening_hours` is a nested object in the request body, not a string, so a `BoundedDayInput`-style local `draft` parses on blur and only calls `onChange` with real JSON, keeping the last-known-good value and showing an inline `openingHours.invalidJson` error on a parse failure instead of ever forwarding one.
  - **`RadioOptionGroup`** (`src/components/RadioOptionGroup.jsx`) — a generic *dynamic* HDS `SelectionGroup` of `RadioButton`s, built from a `{value, label}` option list. `RequestThingPage`'s HOUR-unit reservation flow uses it twice (duration, then start time) — both option lists change with what the member has already picked, which is exactly where the `SelectionGroup` quirks bite (frontend/CLAUDE.md): it centralises the flat-array-of-`id`-carrying-wrappers shape, **and a third quirk this codebase found while building it** — the wrapper's own `id` must differ from its `RadioButton`'s (`${id}-option`, not `id`), or the two elements sharing one `id` break the `<label for>` association and every radio's accessible name goes silently blank, invisible to a bare radio-count assertion. `ReservationRulesFields`' DAY/HOUR selector predates this component and is a fixed two-option set, so it wasn't retrofitted — new dynamic radio groups should use `RadioOptionGroup` rather than reimplement the pattern.
  - **Reservations collection:** when `allowed_thing_types` is exactly `["RESERVE_THING"]` (the user picks "Reservation" in `CollectionForm`'s type select — which makes it exclusive and drops the COMMUNITY mode option, since a reservations collection is always PROPRIETARY), the accordion swaps `RentalRulesFields` for **`ReservationRulesFields`**. Two units, never both (`reservation_unit`, an HDS `SelectionGroup` of two static `RadioButton`s — see the component note below): `reservation_horizon_days` (1–365, how far ahead members can book) and `reservation_max_active_per_member` (1–50, a courtesy cap) apply to either unit and sit above the split; **DAY** (default) then shows `NumberInput`s for `reservation_max_days` (1–7) + the same `WeekdayChips` row (relabelled "days open for reservations"); **HOUR** shows `reservation_max_hours` (1–12) + **`OpeningHoursField`**, a single raw-JSON `TextArea` for the weekly schedule (below). `rental_durations` and `deposit_policy` are cleared from the body for a reservations collection either way; `reservation_max_days`/`reservation_max_hours`+`opening_hours` are sent only for their own unit.
  - **`ClosedDatesField`** sits below whichever rules block shows, for **every** collection: a one-line `TextInput` of comma-separated `DD/MM/YYYY` holidays/closures (`closed_dates`). The page holds the raw text, converts a loaded ISO list with `closedDatesToDisplay`, and sends the text straight through — the backend parses, drops past dates and caps the count. `RequestThingPage`'s pickers grey these days out (`isPickupDisabled` / `reservationPickupDisabled` / `isDateBlocked` all take a `closedDates` array now — no LEND/RENT handoff on one, no RESERVE span across one).
  - **`request_info`** — a `TextArea` (`requestInfo.label` / `.helper`) right after `ClosedDatesField` on both Create and Edit, for **every** collection: a Markdown note for anyone reaching the request page — LEND/RENT/RESERVE alike, not RESERVE_THING only, but never GIFT/SELL, which complete straight from the card and never visit that page. Sent trimmed, unconditionally (unlike `deposit_policy`, which a reservations collection clears). Localizable like every other owner text (`LocalizedInfo variant="requestInfo"`). **512 characters per language** (CA's call, 2026-09: 256 was too short for this one) — the static `.helper` copy sits in its own `<p className="weekday-field-helper">` above the field, same as `RentalRulesFields`' deposit-policy note, and the `TextArea`'s own `helperText` is the live `localizedCounter(requestInfo, 512)` count instead. Past the limit the field turns invalid with `requestInfo.maxLength` (`localizedCounter(...).over`) — a live, in-place warning, not a blocking one: like `deposit_policy`, this field is inside "More options", and `validate()` only ever gates on the visible tier (headline/description/allowed types), so nothing collapsed can silently refuse a submit the reader can't see the reason for. Rendered on `RequestThingPage` — see that page's note below.
  - **`home_page`** — a `TextInput` (`type="url"`, `maxLength={128}`) below `request_info` on both Create and Edit, for **every** collection: the group's own website (`homePage.label` / `homePage.helper`). Sent trimmed; the backend `URLField` rejects a bare word. When set, `CollectionPage`'s hero back link points at it (see CollectionPage above).
  - "Save" button below the form (outside the accordion), then "Delete" button below that (navigates to `DeleteCollectionPage`) — **the one client-side gate on this page** (2026-09): shown only when `userCode === collection.owner`, the strict founder check. Everything else here — Save, Pause, Stats, Export — always rendered with no client gate at all, relying entirely on the server's 403; a co-owner reaching this page now succeeds at every one of them exactly as the founder would, so nothing else needed to change. Delete alone stays owner-only server-side too, so it's the one button not worth showing to someone the API would refuse.
...
- **Stats download** below the Pause section (same border/spacer pattern, design round): a secondary "Download stats (CSV)" button (`GET /api/v1/collections/{code}/stats/` → blob → `{code}-stats.csv`), with an inline error `Notification` on failure. Moved here from the CollectionPage hero — an admin tool belongs with the collection's other curator settings, not in a hero button slot shown on every visit.
- **Collection data export** directly under the stats download (S5, 2026-08): a second secondary button, `GET /api/v1/collections/{code}/export/` → blob → the server-set filename (via `filenameFromResponse`, `src/utils/downloadBlob.js`), same 429/error handling shape. A copy line under it says out loud that this is the whole group, not the CSV summary above, and that it carries other members' emails — a curator should know what they're about to have on their laptop before they click. Owner-or-co-owner server-side (2026-09); a plain member reaching this page by URL gets the same 403 the endpoint always returns.
- **Calendar export** — a third secondary button below the collection export: `POST /api/v1/collections/{code}/calendar-export/` → the collection's upcoming LEND/RENT/RESERVE reservations as a **Google Calendar CSV** (`{code}-calendar.csv`), one all-day event per reservation spanning its block. **POST**, because the call marks the reservations delivered (a mail-scanner GET must not fire it). It is **incremental** — the response header `X-Calendar-Events` gives the count; `> 0` triggers the `downloadBlob` and a success `Notification` naming the count, `0` shows "nothing new since your last download" and downloads nothing. 429 → `common.tooManyAttempts`, any other failure → `calendarExport.error`. The `calendarExport.notice` line under the button states the "only the new ones" promise before any click. Curator-only server-side.
...
- **Two-tier form (progressive disclosure, design round O1)** — identical structure to EditCollectionPage above, minus the status select (Create has none). **The digest select is now present in both** — it used to be Edit-only, which was survivable while `digest_frequency` defaulted to `NONE` and is not now that it defaults to `WEEKLY`: a new collection mails its members, so its owner has to see the field at the moment they create the group. Same options, order and `editCollection.*` keys as Edit, so the one field doesn't read differently on the two screens. The **visible** tier: headline, description (+ `LocalizedInfo`), the mode `RadioButton` group (per-option inline description, `createCollection.modeProprietaryDesc`/`modeCommunityDesc`), and the `CollectionForm` identity cluster. The collapsed **"More options" `Accordion`** holds: tags editor (`TagInput` + `LocalizedInfo variant="tags"`), `RentalRulesFields`, collection-language `Select`, thumbnail `ImageUpload`, welcome-doc `PdfUpload`.
  - **Identity cluster** (`CollectionForm`): the visibility `ToggleButton` (`.toggle-left` wrapper) plus a `Select multiSelect` for allowed thing types — default empty, so the user must explicitly pick at least one, validated live after the first submit attempt. The list is the same in both modes: mode decides WHO may add a thing, not which types. Save fails with 400 from the backend if narrowing would orphan existing things — the response detail names the offending types and is surfaced via Toast.
  - **Rental rules (#7):** `RentalRulesFields` via `utils/rental.js`, in the accordion — a `Select multiSelect` for rental lengths plus the shared **`WeekdayChips`** row for pickup/return days. They save `rental_durations` (days) + `rental_weekdays` (0=Mon…6=Sun). Same `deposit_policy` `TextArea` as EditCollectionPage.
...
- Uses the standard `form-hero` + `Koros` layout with theeeme colors via `useTheeeme(user?.theeeme_colors)` — the hook's override argument is exactly the own-profile-from-API / other-profile-from-localStorage rule this page used to hand-roll, and going through the hook is what gives its two hero buttons the keyboard-focus `*-focus` pins the rest of the app has (`myGroups.test.jsx` pins that).
...
- **`useThingActions`** (`src/hooks/useThingActions.js`) — The **view-model** layer wrapping `useThingBooking`, shared by `ThingLinkbox` and `ThingPage` so the owner-button-matrix / reserve-button logic lives in one place. Derives the type flags (`isOwner`, **`canManage`** — `thing.can_manage`, floor `isOwner`; the matrix, `canDelete` and the `!canManage` reserve-button guard all key on it now — `isCollectionOwner`, `isDateBased`, `needsPage`, `canDelete`, `hasPendingBookings`) and the reserve button's `showButton` / `buttonDisabled` / `loginButtonDisabled` / `buttonLabel` — plus everything `useThingBooking` returns. `canManage` is also what it passes `useThingBooking` to gate the owner-calendar fetch (the backend serves the requester-detail calendar + embedded `bookings` to a manager, not only the owner). The genuine differences are options: `isPaused` (card on a paused collection; the page passes false), `canAct` (the page passes `isAuthenticated`), `loginToAct` (anonymous-on-public — buttons show but each click routes to `/collections/:code/join`), `collectionOwner`, `collectionCode` (forwarded to `useThingBooking`, above), and the `useThingBooking` seeds. `bookingKeepsStatus` (`needsPage || is_endless`) is derived here so callers don't repeat it. Signature: `useThingActions(thing, userCode, options)`.
...
- **`useJoin`** (`src/hooks/useJoin.js`) — The shared `/auth/join/` submit behind the email-capture doors: `MagicLinkJoinPage` (`/share/:token`, plus any a deployment adds) and `JoinToAct` (inside `JoinPage`). They stay separate components because they render very differently — boxed page vs unboxed hero body with inline errors — but the request was byte-identical in both and had already drifted. Owns `email` / `loading` / `status` / `message` plus the CSRF header, the 429 branch, and a **ref-based** re-entry guard — a `loading`-state guard is useless against a second submit in the same tick, which runs the previous render's closure and still sees `false`. **Never sends a `language` field** — it used to, so a brand-new member's first magic link would speak the page's current UI language, but the backend stamped that value onto the new user permanently, and it then outranked the joined collection's own language for every future email to them, with no way back short of a profile edit (CA's report, 2026-09-15). The backend resolves the first magic link from the collection instead — one fewer thing for this hook to send. Options: `sentMessageKey` / `errorMessageKey` (the copy differs per door) and `extraBody` (`JoinToAct`'s `collection_code` plus an optional `thing_code` from the `?thing=` query param, `SharePage`'s `share_token`). Returns `{ email, setEmail, loading, status, message, submit }`.
...
- **`BackLink`** (`src/components/BackLink.jsx`) — Reusable `← {label}` back navigation link. Props: `to`, `label`, and optional `href`. With `href` set it renders an external `<a rel="noopener noreferrer">` (used by `CollectionPage` for a group's `home_page`); otherwise a react-router `<Link to={to}>`.
...
- **`MagicLinkJoinPage`** (`src/components/MagicLinkJoinPage.jsx`) — Shared join landing page, rendered by `SharePage` (and by whatever door a deployment adds): a `PageLayout` hero + email form that POSTs to `/api/v1/auth/join/` (via `useJoin` — see its own note for why it deliberately sends no `language`) and swaps into a sent/error `Notification` (with the "you can close this tab" line on success). Props: `ns` (i18n namespace for the form strings and the `{ns}-email` input id — `'share'` here; a deployment supplies its own through `deploymentI18n`), `docTitleKey` / `titleKey` / `descriptionKey` (full i18n keys — their names differ per page), `docTitleText` / `titleText` / `descriptionText` (literal strings that override the matching `*Key` when set — SharePage fills them from the collection preview, S10), `collectionDescription` (a quiet paragraph under the intro), `extraBody` (extra POST fields, e.g. SharePage's `share_token`), `children` (rendered just under the form, above the `/legal` and `/login` links — upstream `SharePage` passes none; the hosted `/popin` uses it to point at its `/faq`). `JoinToAct` (JoinPage's variant of the same flow) deliberately stays separate — it renders unboxed inside another page's hero and reports errors inline.
...
- **`InboxNotifications`** (`src/components/InboxNotifications.jsx`) — The inbox: one dismissible HDS `Notification` per in-app notification, keyed by `type` (label + body via the `home.*` i18n keys it was born with; payload headlines resolve through `useLocalized`). Owns its own fetch and its dismiss (`DELETE /api/v1/inbox/{code}/`, optimistic). `notificationLink()` returns a `{to, label}` deep link to the object that originated it: `BROADCAST` to the collection (`home.viewCollection`), and **anything carrying a `thing_code` to that thing** (`home.viewThing`) — so a hold request lands the owner on the page where they answer it, and `THING_REPORTED` keeps its old `/things/{code}` target (it carries no collection). `RESERVATION_MADE` / `RESERVATION_CANCELLED` have their own `case`s (label + body from `home.reservation*` keys); both carry `thing_code`, so the deep link falls through the generic "anything with a `thing_code`" branch. **`PROMOTED_CO_OWNER` / `DEMOTED_CO_OWNER`** likewise have `case`s (`home.promoted*` / `home.demoted*`): the backend sends **no email** for either (a deliberate v1 simplification), so the inbox card is the whole notice — a `PROMOTED` says what the member can now do and that their name is now public on the group's page, and both deep-link to `/collections/{collection_code}` (`home.viewGroup`). They fell through to the blank broadcast default until the design round; the `test/inboxNotifications.test.jsx` "every type says something" table now covers them. A response body that isn't a list degrades to "no notifications" rather than taking the page down with it. **`localizedPayload` also fills in every person-name key** (`owner_name`, `questioner_name`, `other_name`, …) with `common.aMember`: the backend sends the bare `name`, never `display_name`, because that fallback is the person's email address and the reader here is a co-member not entitled to it (L2, `core/services/CLAUDE.md`) — so somebody who never filled in a profile arrives as `''`, and every one of these strings interpolates the name mid-sentence. One funnel, so a builder cannot forget. Props: `collection` (optional code → `GET /api/v1/inbox/?collection=`, the owner-only per-collection view on `CollectionPage`; omitted on Home = everything), `reloadKey` (bump to re-fetch), `onNetworkError` (must be stable — it is a fetch-effect dependency).
...
- **`DemoNotice`** (`src/components/DemoNotice.jsx`) — The "this is demo data" banner (HDS `Notification` `type="alert"`) for a seed / onboarding collection. Rendered by `CollectionPage`, `ThingPage`, `RequestThingPage` and `AddThingPage`, each gating on the collection's `is_onboarding` (from `CollectionSerializer`) or a thing's `collection_is_onboarding` (from `ThingSerializer`, resolved through `_viewable_collection`). Like `FeedbackLink`, the copy is the deployment's, not the product's: **renders `null` until `i18n.exists('demoNotice.body')`** — upstream ships no `demoNotice.*` strings, a deployment adds them through `deploymentI18n`. Two lines, from `demoNotice.title` / `.body` / `.realNote`: DESIGN_HOSTED §1 wants it to say both what *is* demo (shared, periodically reset) and what is not (the visitor's own collections), so a warning never scares someone off their own work. No `is_onboarding` field on the create/update serializers — only `seed_demo` sets the flag.
...
| `aboutPath` | `SiteFooter`, `VerifyPage`, `HomePage` | A page saying what this deployment *is*, or `null` for none. Null upstream: what OIUEEI is belongs in the README, and an operator's own pitch belongs to the operator. It decides the footer's about link, the dashboard empty state's second button, and where `landing: "welcome"` sends a new visitor (home, when there is no such page) |
...
**One `deploymentI18n` key set turns a feature on rather than relabelling something: `demoNotice.title` / `.body` / `.realNote`.** Supply them and `DemoNotice` (Shared Components, above) shows a "this is a demo" banner on every page of an `is_onboarding` collection; without them it renders nothing. Documented in `SELF_HOSTING.md` §4.
...
- `TYPE_VALUES` — Array of type value strings (`GIFT`/`SELL`/`RENT`/`LEND`/`RESERVE_THING`; no labels — labels come from i18n).
- `DATE_TYPES` — Types booked by date (`LEND_THING`, `RENT_THING`, `RESERVE_THING` — RESERVE navigates to `RequestThingPage` like the others).
- `FEE_TYPES` — Types where the fee is **required** and shown (`SELL_THING`, `RENT_THING`).
- `FEE_OPTIONAL_TYPES` — Types where the fee is shown but **optional** (`RESERVE_THING`). `ThingForm` shows the input for `FEE_TYPES ∪ FEE_OPTIONAL_TYPES`, `required` only for `FEE_TYPES`.
- `DETAIL_TYPES` — Types with availability/location/condition fields (`GIFT_THING`, `SELL_THING`, `LEND_THING`, `RESERVE_THING` — added for `location`). Deposit is its own gate (`DATE_TYPES` minus RESERVE — a space never leaves).
...
CSS-module class *prefixes* (`Select-module_root`, `selectAndListContainer`,
`listAndInputContainer`, `FieldLabel-module_label`) via `[class*='…']`, since the
hashed suffixes change between builds. Behaviour is pinned by
`ShareCollectionMenu.test.jsx` (one named combobox, menu opens with every
option), but the *look* is not testable in jsdom — on any `hds-react` bump,
re-check the collection hero by eye: a 44px icon, no border/box, and the options
list opening at a readable width anchored to the icon's right edge.
...
3. **That wrapper `id` must NOT be the same string as the `RadioButton`'s own `id`.** Give it a distinct one (`CollectionModeField` and `ReservationRulesFields` both use `${id}-option`) — reusing the radio's own `id` on the wrapper `<div>` puts two elements in the DOM sharing one `id`, and `<label for="…">` then resolves to whichever one it finds first (the empty wrapper), so **every radio's accessible name goes blank** — this passes a bare radio-count assertion (`getAllByRole('radio')` still finds them) and only fails a name-scoped query (`getByRole('radio', { name: … })`), which is exactly why the plan for this feature called for a radio-count test in the first place and why that alone would not have been enough on its own.
