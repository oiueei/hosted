...
- On error: toast notification (top-right, auto-close) — **except** a RESERVE_THING 403 ("you need to be a member of this group to reserve"), which gets a persistent `.invite-nudge` block instead of a toast: the toast auto-closes before the reader can act on it, and this one has an actual fix. The block shows the backend's own message plus a **"Join this group"** button (`handleJoinGroup`) that `POST`s `/api/v1/collections/{code}/join/` — the same signed-in half of login-to-act `CollectionPage`'s `handleJoin` already uses, reused rather than reinvented. `code` covers the collection-context route; the standalone `/things/:code/request` route falls back to `thing.collection_code` (the collection `_viewable_collection` already resolved server-side). On success the block is replaced by a one-line confirmation (`reservation.joinedNowRetry`) and the reader presses the reservation button again themselves — **no auto-retry**, so nothing happens without them choosing it. A failed join shows its own inline error (`collectionPage.visitorJoinError`) and leaves the original reason in place. Any *other* 403 on this endpoint still gets the toast, since joining would not fix one where access was genuinely revoked mid-session.
