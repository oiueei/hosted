...
- **Owner attribution**: guests see "Owner. {name}" (or "Curator. {name}" in COMMUNITY mode) below the description in the hero, linking to `/{owner_code}` (the founder's public profile) — always the single `owner`, never a co-owner. Uses `owner_name` from `CollectionSerializer`.
- **Co-curators line (2026-09)**: right below the attribution, when `collection.co_owners.length`, a "Co-curators. {names}" line links each one to their profile — public, read from the `co_owners` list `CollectionSerializer` now serves to every viewer (the same reasoning as `owner_name`: a curator is someone the group's own page already names).
...
- Simple page with h1 title:
  ...
  - **Promote/demote (owner-only)**: on each accepted (non-pending) row, the founder — and only the founder — additionally gets a star toggle (`IconStar`/`IconStarFill` `TooltipButton`, "Make co-curator" / "Remove co-curator status") that `POST`s or `DELETE`s `/api/v1/collections/{code}/co-owners/` with `{user_code}`, then reloads the collection so the roster's badge and the co-owner set stay the server's own word.
