...
| `faqPath` | `LoginPage` | Where this deployment's help/FAQ page lives, or `null` for none. Linked from `/login`, the door with the most traffic — the FAQ (pricing, early-bird terms, who operates the service) is deployment-specific content with no home upstream. Rendered right under the "trouble signing in?" line, same `null`-means-no-link shape as `aboutPath` (S4, 2026-08) |
