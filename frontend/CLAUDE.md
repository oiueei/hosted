...
- **Hero back link ("← Home"):** normally an internal `<Link to="/">`. When `collection.home_page` is set and `sanitizeUrl` accepts it (http/https only), `BackLink` gets an `href` instead and renders a plain external `<a rel="noopener noreferrer">` pointing at the group's own site. A non-http value falls back to the internal link.
