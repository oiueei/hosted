...
- On error: toast notification (top-right, auto-close) — **except** RESERVE_THING's "not a member" 403, handled by `joinThenRetry` (CA's call): that 403 can only happen on a **PUBLIC** collection (`can_view` already gated everything else, and a non-member reaches it only there), so joining is always genuinely possible and was never really a choice being offered — the page auto-joins and silently retries the *exact same* reservation, with no second click and no "you're in, try again" step in between. It `POST`s `/api/v1/collections/{code}/join/` — the same signed-in half of login-to-act `CollectionPage`'s `handleJoin` already uses, reused rather than reinvented (`code` covers the collection-context route; the standalone `/things/:code/request` route falls back to `thing.collection_code`, the collection `_viewable_collection` already resolved server-side). **Only a failure of the auto-join itself surfaces anything**: a persistent `.invite-nudge` block (not a toast, which would auto-close before the reader could act) shows the backend's original reason plus a manual **"Join this group"** fallback button (`handleJoinGroup`) that retries the same join-then-reserve sequence against `lastBodyRef` — the exact body `handleSubmit` already built, so nothing has to be re-typed. A failed retry shows its own inline error (`collectionPage.visitorJoinError`) and leaves the original reason in place. Any *other* 403 on this endpoint still gets the plain toast, since joining would not fix one where access was genuinely revoked mid-session.
