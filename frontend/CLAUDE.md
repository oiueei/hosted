...
- **Form fields** adapt to thing type:
  ...
  - `RESERVE_THING` — **pickup date + duration in days** (1..`thing.reservation_max_days`; the duration `Select` is omitted and the length fixed to 1 when the cap is 1), plus an optional **`project_note` `TextArea`** (≤512). The date picker's `maxDate` is `today + thing.reservation_horizon_days` (the collection's own limit, not the fixed 90 the rental picker uses). Pickup picker uses `reservationPickupDisabled` (`utils/rental.js`) — stricter than `isPickupDisabled`: **every** day of the span must be an allowed weekday, since the space can't be booked across a closed day. POSTs `{ start_date, duration_days, project_note, collection_code }`. On success the notice reads *"Your reservation is confirmed"* — it is auto-confirmed, so there is no "we've let the owner know".
