...
| `PROPOSAL_APPROVE` | `_handle_proposal_approve` | **POST** | Sends the real invitation, behind the **same** quota + member-ceiling guard as the in-app approval (`invitation_service.proposal_approval_blocked` — the link is not a way around either; a refusal leaves both links alive so the owner can answer tomorrow, and carries `retryable: true` to say so — a full collection and a settled suggestion both answer 400, so the SPA cannot tell "not now" from "gone" without it). GET previews only `{action, requires_confirmation}` — approving **mails a third party**, so a link scanner must never fire it, and the preview carries **none of the proposal's details** (the address, the note, the proposer's name whose display fallback is *their* email): a GET is what a link scanner issues, and the person suggested does not know they were. The SPA auto-commits with a POST and reads the invited address from that response |
...
4. `get_or_create` user by email. **No `language` is ever taken from the request** — a newly created user's `language` stays blank, so `resolve_email_language` falls through to the joined collection's own language for every email to them, including this first one. This used to accept the visitor's current UI language and stamp it onto the new user permanently, which then outranked the collection's own language for every future email to that member, with no way back short of a profile edit (CA's report, 2026-09-15).
...
The **name, description and language** of the collection a `/share/{token}` link opens, so `SharePage` can render "Join the Chalmercadillo" instead of "Join us on OIUEEI" — a stranger handed the link on WhatsApp has no idea what OIUEEI is. Returns **only** `{headline, description, language}`. `headline`/`description` are **raw** (either may be a `{lang: text}` map the SPA resolves against the reader's language, O6 — same as every other collection read); `language` (added 2026-09-15) feeds `useCollectionLanguage` — this join door is reached before a stranger is a member of anything, so the collection's own language is the only signal there is yet, and it is no more sensitive than the other two. Never the owner, the roster, a member count or even the collection code.
...
4. **CSRF (cookie auth)** — because the access token rides in a cookie, `CookieJWTAuthentication` runs DRF's CSRF check (`enforce_csrf`, mirroring `SessionAuthentication`) for **cookie-authenticated unsafe methods** — defence in depth behind the cookie's `SameSite=Lax`. Bearer-header auth is exempt (the header is never sent cross-site), so API clients and the Bearer-token test suite are unaffected. `MeView` GET sets the `csrftoken` cookie via `@ensure_csrf_cookie` (hit on every app load); the SPA reads it and sends it as `X-CSRFToken` on every unsafe request. **The decorator sits on `dispatch`, not `get`** — it forces the cookie by calling `get_token()` from inside the method it wraps, and DRF's `APIView.dispatch()` runs `check_permissions()` (raising 401 for `IsAuthenticated` before `get()` is ever called) — so decorating `get()` directly meant the cookie was never set for exactly the visitor who needs it most: someone brand new, whose first-ever hit here 401s because `App.jsx`'s app-load warm-up fires before the magic-link GET that authenticates them completes. That silently broke the first unsafe request (a reservation, a question) from anyone whose magic link drops them straight onto one thing (S13) with no earlier authenticated page ever calling this endpoint successfully. Pinned by `test_me_get_sets_csrftoken_cookie_even_when_it_401s`. The test client disables the check by default (`enforce_csrf_checks=False`), so only `test_csrf.py` (which opts in) exercises it.
...
- `/health/` GET+HEAD — 60 requests per minute per IP. The only anonymous endpoint that reaches the database on every hit, so uncapped it is DB amplification rather than a monitor. Far above a real monitor's cadence (5 minutes = 0.2/m). It is a plain Django view, so it answers **429 itself** rather than letting `Ratelimited` surface as Django's 403 — DRF's exception handler doesn't run here. **The limiter is called by hand (`is_ratelimited`, inside a `try`), not through `@ratelimit`** (2026-09-21): its counter lives in `CACHES['default']`, which is `DatabaseCache` — the same Postgres the endpoint checks — so the outage `/health/` exists to report is also the one that breaks the limiter. As a decorator it ran before the body, and a refused connection escaped from it as an unhandled 500 (production, 2026-09-16, UptimeRobot's HEAD probe → Sentry) instead of the designed `503 degraded`. A limiter that cannot reach its counter now falls through to the real check. `core/tests/unit/test_health.py` pins it with a cache backend that refuses every operation (`RefusedCache`) — the earlier tests faked only the view's own cursor and left the limiter's cache answering, a state production never reaches, which is why they stayed green through the real outage. The group is kept as `core.urls.health_check`, the name the decorator derived, so live counters carry over
