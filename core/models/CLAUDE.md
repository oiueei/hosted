...
| `language` | CharField(2) | No | The language OIUEEI writes to this user in (`es`/`ca`/`en`) — **the strongest level of the email language hierarchy**, beating both the collection's language and the deployment default. Blank (default) = inherit. Saved from the language `Select` in `EditProfilePage` (which also switches the UI), and stamped on **newly created** users from the `language` they were reading the join page in. |
