...
| `request_info` | CharField(1024) | No | A short note for anyone about to request anything from this collection — **every verb (GIFT/SELL/RENT/LEND/RESERVE), not RESERVE_THING only** ("bring ID", "reservations must be confirmed 24h ahead"). **256 per language, 1024 stored**, same trick as `deposit_policy`/`description`, `LocalizedTextField` in the serializer. Rendered as Markdown on the frontend's `RequestThingPage`, below the title (`ThingSerializer.get_collection_request_info` resolves it through `_viewable_collection`). Default `""`. |
