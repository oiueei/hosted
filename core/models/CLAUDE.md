...
| `request_info` | CharField(2048) | No | A short note for anyone about to request anything from this collection — **every verb (GIFT/SELL/RENT/LEND/RESERVE), not RESERVE_THING only** ("bring ID", "reservations must be confirmed 24h ahead"). **512 per language, 2048 stored** (CA's call, 2026-09: 256 was too short for this one) — same trick as `deposit_policy`/`description`, `LocalizedTextField` in the serializer with an explicit `storage_max_length=2048` (wider than `LOCALIZED_TEXT_STORAGE`'s default 1024). Rendered as Markdown on the frontend's `RequestThingPage`, below the title (`ThingSerializer.get_collection_request_info` resolves it through `_viewable_collection`). Default `""`. |
