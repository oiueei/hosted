...
| `reservation_max_hours` | PositiveSmallIntegerField | No | **Dormant since 0150** — replaced by `reservation_min_minutes`/`reservation_max_minutes` below (an early adopter wanted reservations shorter than an hour, which a whole-hours field can't express). Kept as a column, unread and unwritten by anything past the migration that copied its value forward; dropped in a later one. |
