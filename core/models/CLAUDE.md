...
| `reservation_max_hours` | PositiveSmallIntegerField | No | **`HOUR`-unit reservations collections only.** The longest a single reservation may run, in hours, 1–12 (serializer-enforced). Default `3`. **Exempt for a full-day reservation** — one spanning exactly the first opening block's start to the last one's end for that weekday, gap included — since "book the whole day" has to mean the whole day even when that is more hours than any single slot may otherwise run; see `Collection.reservation_hour_violation`. Inert under `DAY` (`reservation_max_days` governs there). |
...
- `reservation_hour_violation(start_date, start_time, end_time, today=None)` - The `HOUR`-unit twin of `reservation_violation`: an error string if `[start_time, end_time)` on `start_date` breaks the collection's hourly rules, else `None`. A valid span is either **entirely inside one opening block**, or **exactly the full day** (first block's open to last block's close, any gap included) — the second form is exempt from the `reservation_max_hours` cap, since a full-day booking has to mean the whole day. Anything else (a span crossing a gap without covering the whole day, one landing off a block boundary) is refused rather than clipped. Horizon and `closed_dates` checks mirror `reservation_violation` exactly, in hours instead of days. The **single** backstop `booking_service.request_reservation`'s `HOUR` branch relies on.
