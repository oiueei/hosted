...
| `reservation_unit` | CharField(4) | No | **Reservations collections only.** `DAY` (default) or `HOUR` — never both. `DAY` is the original shape: a reservation occupies one or more whole days (`reservation_max_days`, `rental_weekdays` reused as "days open"). `HOUR` books a single day in slots within that day's `opening_hours`, capped at `reservation_max_hours`. **Dormant as of migration 0147**: nothing yet reads this field to change behaviour — every reservation today is still whole-day, regardless of its value. |
| `opening_hours` | JSONField (dict) | No | **`HOUR`-unit reservations collections only.** Per-weekday opening windows, keyed by Django's string form of `weekday()` (`"0"`–`"6"`, Mon–Sun), each a list of non-overlapping `["HH:MM", "HH:MM"]` pairs — e.g. `{"0": [["10:00","14:00"],["16:00","20:00"]], "6": []}`. An empty list (or an absent key) means closed that day, **replacing** `rental_weekdays` as "which days are open" under `HOUR` — the two would otherwise be able to disagree. Default `{}` (every day closed until the owner sets hours). **Dormant as of migration 0147**. |
| `reservation_max_hours` | PositiveSmallIntegerField | No | **`HOUR`-unit reservations collections only.** The longest a single reservation may run, in hours, 1–12. Default `3`. Governs which of the duration radio options (1h/2h/3h/half day/full day) a member is offered — "half day"/"full day" appear only when they fit under this cap. Inert under `DAY` (`reservation_max_days` governs there). **Dormant as of migration 0147**. |
...
| `start_time` | TimeField | No | **`HOUR`-unit RESERVE_THING only.** `NULL` means "whole day" — true of every LEND/RENT/DAY-unit-RESERVE booking, always. A `HOUR`-unit reservation still carries `end_date = start_date + 1` (it never spans midnight); this narrows that one day to a slot within it. **Dormant as of migration 0147** — nothing yet writes or reads it. |
| `end_time` | TimeField | No | **`HOUR`-unit RESERVE_THING only.** Same `NULL`-means-whole-day rule as `start_time`. **Dormant as of migration 0147**. |
