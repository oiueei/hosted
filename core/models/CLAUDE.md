...
- `reservation_violation(start_date, duration_days)` - The RESERVE_THING backstop, parallel to `rental_violation`: an error string if `duration_days` is outside `1..reservation_max_days` or any day of `[start, start+duration)` falls outside `rental_weekdays` (when set), else `None`. Called by `booking_service.request_reservation`.
