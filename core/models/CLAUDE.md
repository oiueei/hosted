...
| `reservation_unit` | CharField(4) | No | **Reservations collections only.** `DAY` (default) or `HOUR` — never both. `DAY` is the original shape: a reservation occupies one or more whole days (`reservation_max_days`, `rental_weekdays` reused as "days open"). `HOUR` books a single day in slots within that day's `opening_hours`, capped at `reservation_max_hours` — see `Collection.reservation_hour_violation`. |
...
| `reservation_max_hours` | PositiveSmallIntegerField | No | **`HOUR`-unit reservations collections only.** The longest a single reservation may run, in hours, 1–12 (serializer-enforced). Default `3`. **Applies to the full-day span too, with no exception** (CA's call — exempting it was offered and turned down): a venue whose day adds up to more hours than this cap simply never offers "the whole day" as a bookable span; the owner raises the cap if they want it offered. See `Collection.reservation_hour_violation`. Inert under `DAY` (`reservation_max_days` governs there). |
