...
| `request_reservation(thing, requester, owner_email, start_date, duration_days, project_note="", collection_code=None)` | RESERVE | 403 unless the requester is a collection member; `Collection.reservation_violation()` (400); horizon check; `has_overlap()` (409). Creates the `ACCEPTED` booking, sends the requester's confirmation and — to **every curator** of the reservations collection, minus the requester if they are one (`_reservation_curators`, deduped) — the notice email + `RESERVATION_MADE` in-app. Logs `HOLD_REQUESTED` **and** `HOLD_ACCEPTED` (the funnel is instant). |
| `cancel_reservation(booking, by_user)` | RESERVE | Allowed to the **requester or any curator** of the reservations collection (`Thing.can_manage`, 2026-09; rule 4), only while `start_date >= today`. Sets `CANCELLED` under a lock, then fans `RESERVATION_CANCELLED` (in-app + email) to everyone who didn't cancel — the member (if a curator did it) and every other curator. `other_name` / the email's `{other}` is always **whoever actually cancelled**; `cancelled_by_owner` (→ the email's "your reservation" wording) is set only on the member's copy. |
| `_reservation_curators(collection)` | — | The reservations collection's owner + co-curators, deduped by code. `[]` for `None`. The one place the "who runs this space" list is built. |
...
| **Cat. 2 — Activity** | `CATEGORY_ACTIVITY` | `User.notify_activity` | `send_booking_request_email`, `send_booking_decision_email`, `send_booking_confirmation_email`, `send_reservation_confirmed_email`, `send_reservation_notice_email`, `send_reservation_cancelled_email`, `send_invite_rejected_email`, `send_faq_question_email`, `send_faq_answer_email`, `send_faq_hide_email`, `send_thing_reported_email`, `send_return_reminder_email`, `send_return_due_email`, `send_broadcast_email`, `send_invitation_proposal_email`, `send_proposal_declined_email` |
...
- **Reservation emails** (`send_reservation_*`): auto-confirmed, so the requester reads "confirmed", not "request sent"; the **notice** (`send_reservation_notice_email`) goes to **every curator** of the reservations collection (2026-09, `_reservation_curators` — the team runs the space together), carrying the `project_note` (escaped) when present; the cancelled one picks "your reservation" vs "their reservation" by `cancelled_by_owner`, which the fan-out sets true only on the member's copy. RESERVE does not use the three generic booking emails.
