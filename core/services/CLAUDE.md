...
| `request_reservation(thing, requester, owner_email, start_date, duration_days, project_note="", collection_code=None)` | RESERVE | 403 unless the requester is a collection member; `Collection.reservation_violation()` (400); horizon check; `has_overlap()` (409). Creates the `ACCEPTED` booking, sends both emails + the owner's `RESERVATION_MADE` notice, logs `HOLD_REQUESTED` **and** `HOLD_ACCEPTED` (the funnel is instant). |
| `cancel_reservation(booking, by_user)` | RESERVE | Allowed to the **requester or the owner** (rule 4), and only while `start_date >= today`. Sets `CANCELLED` under a lock (frees the slot), then in-app + email `RESERVATION_CANCELLED` to whichever party did not cancel. |
...
- **Reservation emails** (`send_reservation_*`): auto-confirmed, so the requester reads "confirmed", not "request sent"; the owner's notice carries the `project_note` (escaped) when present; the cancelled one picks "your reservation" vs "their reservation" by `cancelled_by_owner`. RESERVE does not use the three generic booking emails.
