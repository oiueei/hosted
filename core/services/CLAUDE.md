...
| `compute_hourly_availability(blocked_periods, collection, today=None)` | same booking iterable, plus the `HOUR`-unit collection itself | The `HOUR`-unit twin, not a modification of the function above — same `(available_today, next_available)` shape so `Thing.availability_window` treats both units alike. "Available" means *some* opening block that day still has a free 60-minute gap (via the private `_day_has_a_free_hour`, which sweeps that day's bookings — a whole-day one, `start_time` NULL, closes the day outright — against `collection.day_opening_blocks()`); it does not check whether a *specific* duration/start-time combination is offered, which `RequestThingPage` works out itself once a day is picked. Walks `collection.reservation_horizon_days` ahead, same shape as the day-based walk. |
