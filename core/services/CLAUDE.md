...
| `request_reservation(thing, requester, owner_email, start_date, duration_days, project_note="", collection_code=None)` | RESERVE | 403 unless the requester is a collection member; `Collection.reservation_violation()` (400); horizon check; `has_overlap()` (409). Creates the `ACCEPTED` booking, sends the requester's confirmation and — to **every manager** of the thing (`Thing.managers`, deduped), minus the requester if they are one — the notice email + `RESERVATION_MADE` in-app. Logs `HOLD_REQUESTED` **and** `HOLD_ACCEPTED` (the funnel is instant). |
