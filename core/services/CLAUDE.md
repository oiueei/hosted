...
|---|---|
| `Capabilities` | Frozen dataclass: `collection_modes`, `thing_types`, `request_url`, `co_owners_enabled`. `as_dict()` is the JSON shape served on `GET /auth/me/` (lists, not tuples). `request_url` is where to go and ask; `None` means there is nowhere, which is the standalone's answer because there is nothing to ask for. `co_owners_enabled` (default `True`) is whether this deployment lets a COMMUNITY collection's owner promote a member to co-owner at all — see `co_owners_denial` below. |
...
| `co_owners_denial(user, capabilities=None)` | Why this deployment won't let `user` promote a co-owner, else `None`. Same shape as the two denials above — enforced at the one door that creates co-owner status, `CollectionCoOwnerView.post`. **Grandfathered like every other capability here**: it gates *promoting*, never *demoting* — an owner can always undo a co-owner they already appointed, even on a deployment that has since disabled the feature. |
