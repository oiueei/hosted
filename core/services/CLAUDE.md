...
| **Cat. 3 — News** | `CATEGORY_NEWS` | `User.notify_news` | `send_digest_email` |
...
- **Multi-recipient**: functions that take `emails=[...]` (digest, broadcast) use `_filter_recipients()` for a bulk query that drops opted-out addresses before iterating.
